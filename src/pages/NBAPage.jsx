...
export default function NBAPage() {
  ...
  useEffect(() => {
    async function fetchGames() {
      try {
        setLoading(true);
        const response = await getCachedMarketData({ sport: "NBA" });
        const data = response?.data;
        
        if (data?.success && Array.isArray(data.games)) {
          setGames(data.games);
          console.log(`Loaded ${data.games.length} NBA games`);
        } else {
          setGames([]);
          setError(data?.message || "No NBA games available");
        }
      } catch (err) {
  ...
  const handleAnalyzeGame = async (game, analysisType = 'game') => {
    try {
      console.log(`🧠 Analyzing ${analysisType} for NBA game:`, game.id);
  ...
  return (
    <div className="p-6 bg-gradient-to-br from-slate-900 to-slate-800 min-h-screen">
      <div className="max-w-7xl mx-auto">
        <h1 className="text-3xl font-bold text-white mb-2">NBA</h1>
        ...
        {loading ? (
          <div className="flex items-center justify-center h-64">
            ...
            <p className="ml-4 text-white">Loading NBA games...</p>
          </div>
        ) : error ? (
          ...
          <div className="text-center py-12 bg-slate-800 rounded-lg shadow-lg border border-slate-700">
            <p className="text-cyan-400 text-lg font-semibold">No NBA games available</p>
